...
1. Runtime 单元测试证明算法题生成 Prompt 要求参考答案按“1. 解题思路、2. 口述要点、3. 代码示例”组织，多题按配置上限拆为有序批次并行生成，同时模型输出会被结构化校验，非法来源链接和不一致测试参数会被拒绝。
2. Backend 测试证明 `/questions/generate` 只返回无 `questionId` 的候选题且不调用 Repository 保存。
3. 前端测试证明算法题与问答题使用不同表单和默认提示，算法生成来源页不再显示或提交 LeetCode 链接字段，并以响应式资料区展示上传、参考文本和出题要求；问答题型切换会显示对应选项或评分字段。
