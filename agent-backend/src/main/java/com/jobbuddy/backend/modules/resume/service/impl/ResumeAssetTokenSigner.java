...
/**
 * 简历附件资源的签名令牌工具：签发带过期时间的 HMAC 令牌，并在读取时校验签名、
 * 归属用户与对象路径，避免对象存储路径直接暴露给前端。
 */
class ResumeAssetTokenSigner {

    static final java.util.Set<String> ALLOWED_ASSET_SUFFIXES = new java.util.HashSet<String>(
    ...
    String requireAssetObjectName(String token, String userId) {
        ...
        boolean legacyDefaultAsset = legacyUser.equals(tokenUser) && !legacyUser.equals(effectiveUser);
