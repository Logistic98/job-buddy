...
/**
 * SSE 会话主编排：负责连接生命周期、意图分流与各业务链路的调度；
 * 事件下发、持久化、记忆写入与各链路细节由同包协作类承载。
 */
@Service
public class ChatSseServiceImpl implements ChatSseService {
    ...
    private final ConcurrentMap<SseEmitter, AtomicBoolean> emitterCancelled = new ConcurrentHashMap<SseEmitter, AtomicBoolean>();
    ...
    public ChatSseServiceImpl(JobRuntimeService jobRuntimeService,
        ...
        this.persistence = new ChatPersistenceCoordinator(sessionStore, namedThreadFactory("chat-persist"));
        ...
        this.requestFactory = new RuntimeManagedRequestFactory(integrationService, personalContextBuilder, properties);
        CurrentResumeLoader resumeLoader = new CurrentResumeLoader(resumeStorageService);
        this.selectedJobAnalysisHandler = new SelectedJobAnalysisHandler(sender, resumeLoader, resumeStorageService, integrationService, requestFactory);
        this.resumeFlowHandler = new ResumeFlowHandler(sender, resumeLoader, resumeStorageService, jobRuntimeService, integrationService, requestFactory);
        this.jobRecommendHandler = new JobRecommendHandler(sender, persistence, jobRuntimeService, properties);
        this.runtimeManagedTaskHandler = new RuntimeManagedTaskHandler(sender, integrationService, requestFactory);
    }
    ...
    public SseEmitter stream(final ChatStreamRequest request) {
        ...
        final long emitterTimeoutMillis = agentServiceProperties.getStreamReadTimeout().toMillis() + 10000L;
        ...
        emitter.onTimeout(new Runnable() {
            ...
            public void run() {
                cancelled.set(true);
                log.warn("SSE 连接超时（{}ms），取消后台任务 sessionId={}", emitterTimeoutMillis, request.getSessionId());
                cancelTask(taskRef);
            }
        });
        emitter.onError(new java.util.function.Consumer<Throwable>() {
            ...
            public void accept(Throwable throwable) {
                ...
                log.debug("SSE 连接异常（客户端可能已断开）sessionId={}: {}", request.getSessionId(), throwable.getMessage());
                cancelTask(taskRef);
            }
        });
        Future<?> future = executor.submit(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (cancelled.get()) {
                        // 连接已超时/断开导致的任务中断属预期终止，不按业务异常告警。
                        log.debug("SSE 任务因连接关闭而终止 sessionId={}: {}", request.getSessionId(), e.getMessage());
                    } else {
                        log.warn("SSE 会话处理异常: {}", e.getMessage(), e);
                        try {
                            sender.send(emitter, "error", Collections.singletonMap("message", e.getMessage()));
                            sender.send(emitter, "done", Collections.singletonMap("ok", false));
                        } catch (Exception sendError) {
                            // 客户端可能已断开，写 SSE 失败属预期，debug 留痕即可。
    ...
    private void handle(ChatStreamRequest request, SseEmitter emitter) throws IOException {
        ...
        if (isSelectedJobAnalysis(request)) {
            ...
            selectedJobAnalysisHandler.handle(emitter, sessionId, state, request.getMessage(), request.getSelectedJob());
            return;
        }

        // 扫码登录后的续跑：复用上一轮检索条件，跳过任务理解直接继续岗位搜索，与登录提示合并为同一段连续过程。
        if (resumeAfterAuth) {
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("auth_resume", "登录后继续执行", "success", "Boss 登录完成，继续岗位搜索。", state.lastSlots));
        ...
        if (flipJobs) {
            ...
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("job_flip", "换一批", "success", "复用上一轮检索条件，直接翻到第 " + nextPage + " 批岗位。", flipSlots));
        ...
        sender.send(emitter, "tool_status", toolStatus("runtime_understanding", "Runtime 任务理解", "running", "已收到请求，正在理解你的问题并准备作答。", null));
        ...
        if (!resumeAfterAuthRequested && !flipJobsRequested) {
            persistence.appendMessageAsync(sessionId, "user", request.getMessage(), null);
            memoryWriter.captureLongTermMemoryAsync(request.getMessage());
        }
        ...
        sender.sendToolStatus(emitter, sessionId, state, toolStatus("request_init", "初始化会话", "success", "会话已建立，准备调用 Agent Runtime。", null));
        ...
        if (isSafetyGateBlocked(preIntent)) {
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("intent_safety_gate", "高风险拦截", "error",
                    "该请求被独立安全门控判定为高风险并拒绝执行。", preIntent));
            sender.sendAssistant(emitter, sessionId, state, "抱歉，该请求被判定为高风险，已被安全策略拒绝，无法继续执行。",
        ...
        sender.sendToolStatus(emitter, sessionId, state, toolStatus("runtime_understanding", "Runtime 任务理解", "success", intent.getDomain() + "/" + intent.getIntent() + "，置信度 " + intent.getConfidence(), directive));

        if (isCapabilityUnavailable(directive)) {
            String answer = stringValue(directive.get("answer"), "该能力尚未接入执行链路。");
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("capability_not_implemented", "能力未实现", "error", answer, directive.get("implementation")));
            sender.sendAssistant(emitter, sessionId, state, answer, Collections.<String, Object>singletonMap("runtimeDirective", directive));
            return;
        }
    ...
    private void handleDirective(SseEmitter emitter, String sessionId, String rawMessage, ChatSessionState state, Map<String, Object> directive, IntentResult intent) throws IOException {
        ...
        if (matchesCapability(action, intent, "call_login", "trigger_boss_login", "auth.login")) {
            ...
            if (!Boolean.TRUE.equals(login.get("authRequired"))) {
                sender.sendAssistant(emitter, sessionId, state, "Boss 登录态有效，可继续筛选岗位或查看详情。", Collections.<String, Object>singletonMap("runtimeDirective", directive));
                return;
            }
