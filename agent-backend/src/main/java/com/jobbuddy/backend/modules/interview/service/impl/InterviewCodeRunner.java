 ...
 * 编程题运行器。
 *
 * <p>不在 agent-backend 进程内执行用户代码。后端只负责组装判题 Harness，并统一提交给 agent-sandbox，由 agent-sandbox 基于
...
public class InterviewCodeRunner {
  ...
  private Map<String, Object> runInSandbox(
      String language, String source, String functionName, List<Map<String, Object>> tests)
      throws Exception {
    ...
    try {
      ...
      return failure(
          "agent-sandbox 调用失败："
              + compact(e.getResponseBodyAsString(), "HTTP " + e.getRawStatusCode()));
  ...
  private String buildSandboxOrchestrator(
      String language, String source, String functionName, List<Map<String, Object>> tests)
      throws Exception {
    ...
    code.append("TESTS_B64 = ")
        .append(pyString(base64(objectMapper.writeValueAsString(tests))))
        .append("\n");
    ...
    code.append(
        "        emit({'passed': False, 'rows': [], 'message': first_line((proc.stderr or '') + '\\n' + (proc.stdout or ''), '运行失败')})\n");
    code.append(
    ...
    code.append("try:\n");
    code.append(
        "    with tempfile.TemporaryDirectory(prefix='job-buddy-practice-', dir=os.getcwd()) as workspace:\n");
    code.append("        if LANGUAGE == 'python':\n");
    code.append("            write(os.path.join(workspace, 'main.py'), CODE)\n");
    code.append(
        "            proc = subprocess.run(['python3', 'main.py'], cwd=workspace, input=TESTS, capture_output=True, text=True, timeout=TIMEOUT_SECONDS)\n");
    code.append("            parse_child(proc)\n");
    code.append("        elif LANGUAGE == 'javascript':\n");
    code.append("            write(os.path.join(workspace, 'main.js'), CODE)\n");
    code.append(
        "            proc = subprocess.run(['node', 'main.js'], cwd=workspace, input=TESTS, capture_output=True, text=True, timeout=TIMEOUT_SECONDS)\n");
    code.append("            parse_child(proc)\n");
    ...
    code.append(
        "            compile_proc = subprocess.run(['javac', 'Solution.java', 'Runner.java'], cwd=workspace, capture_output=True, text=True, timeout=TIMEOUT_SECONDS)\n");
    code.append("            if compile_proc.returncode != 0:\n");
    code.append(
        "                emit({'passed': False, 'rows': [], 'message': first_line((compile_proc.stderr or '') + '\\n' + (compile_proc.stdout or ''), '编译失败')})\n");
    code.append(
        "            proc = subprocess.run(['java', '-cp', workspace, 'Runner'], cwd=workspace, input=TESTS, capture_output=True, text=True, timeout=TIMEOUT_SECONDS)\n");
    code.append("            parse_child(proc)\n");
    code.append("        else:\n");
    code.append(
        "            emit({'passed': False, 'rows': [], 'message': '当前仅支持 Python、Java、JavaScript 运行样例'})\n");
  ...
  private Map<String, Object> sandboxPolicy() {
    ...
    // agent-sandbox 会为每次请求创建独立临时工作区；allowWrite 为空表示沿用该工作区白名单。
    // 不能请求宿主机 /tmp，否则与请求工作区取交集后会得到空白名单，导致判题无法创建子进程文件。
  ...
  private String pythonHarness(String functionName) {
    return "import json, sys\n"
        ...
        + "    fn = getattr(solution_class(), '"
        + functionName
        + "', None) if isinstance(solution_class, type) else None\n"
        + "rows = []\n"
        + "for test in tests:\n"
        + "    has_expected = 'expected' in test\n"
        + "    row = {'name': test.get('name', '用例'), 'input': _stable(test.get('args', []))}\n"
        + "    if has_expected: row['expected'] = _stable(test.get('expected'))\n"
        + "    try:\n"
        + "        if not callable(fn): raise Exception('未找到函数或 Solution 方法："
        + functionName
        + "')\n"
  ...
  private String javascriptHarness(String functionName) {
    return "function stable(value){ return JSON.stringify(value); }\n"
        ...
        + "  try {\n"
        + "    if (typeof fn !== 'function') throw new Error('未找到函数："
        + functionName
        + "');\n"
        + "    const actual = fn.apply(null, JSON.parse(JSON.stringify(test.args || [])));\n"
        + "    row.actual = stable(actual); row.passed = !hasExpected || stable(actual) === stable(test.expected);\n"
