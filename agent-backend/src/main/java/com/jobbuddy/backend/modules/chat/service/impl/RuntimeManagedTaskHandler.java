...
/**
 * Runtime 托管任务链路：流式优先下发答案与推理增量，空产出时回退非流式托管调用，
 * 并按流式中断/无产出/成功分别下发对应终态。
 */
class RuntimeManagedTaskHandler {
    ...
    RuntimeManagedTaskHandler(ChatSseEventSender sender,
    ...
    void handle(final SseEmitter emitter, final String sessionId, String rawMessage, ChatSessionState state, Map<String, Object> directive, IntentResult intent) throws IOException {
        Map<String, Object> detail = new LinkedHashMap<String, Object>();
        detail.put("directive", directive == null ? Collections.emptyMap() : directive);
        detail.put("intent", intent);
        sender.sendToolStatus(emitter, sessionId, state, toolStatus("runtime_managed", "Runtime 托管任务", "running", "Agent Runtime 正在生成结果。", detail));

        Map<String, Object> metadata = requestFactory.runtimeManagedMetadata(rawMessage, state, directive, intent);
        Map<String, Object> request = requestFactory.buildRuntimeManagedRequest(sessionId, rawMessage, "job-buddy", metadata, true);
        ...
        if (answer.isEmpty() && !streamFailed) {
            ...
            Map<String, Object> fallback = requestFactory.runRuntimeManagedAnswerWithProfile(sessionId, rawMessage, "job-buddy", metadata);
        ...
        if (!hasAnswer) {
            ...
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("runtime_managed", "Runtime 托管任务未产出", "error", reason, resultDetail));
            sender.sendAssistant(emitter, sessionId, state, reason, runtimeResult.isEmpty() ? null : Collections.<String, Object>singletonMap("runtimeResult", resultDetail));
            return;
        }
        if (streamFailed) {
            // 已流式展示部分内容但中途报错：保留已下发文本，但以错误态提示结果可能不完整，避免把残缺回答当成功。
            String reason = "Runtime 流式中断，已展示内容可能不完整：" + streamError;
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("runtime_managed", "Runtime 托管任务中断", "error", reason, resultDetail));
        } else {
            sender.sendToolStatus(emitter, sessionId, state, toolStatus("runtime_managed", "Runtime 托管任务完成", "success", "Runtime 已返回回答。", resultDetail));
        }
