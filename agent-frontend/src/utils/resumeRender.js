...
export function renderResumeMarkdown(source) {
  ...
  const closeContainer = () => {
    ...
    if (current === 'left') {
      html.push('</div>')
      pendingLeftWrapper = true
    } else if (current === 'right') {
      html.push('</div></div>')
      pendingLeftWrapper = false
    } else if (current === 'header') html.push('</section>')
    else if (current === 'title') html.push('</div>')
    else if (current) html.push('</div>')
  }
  for (const raw of lines) {
    ...
    if (/^:{3,5}\s*left\s*$/.test(trimmed)) {
      beforeNormal()
      html.push('<div class="lr-container"><div class="left">')
      stack.push('left')
      continue
    }
    if (/^:{3,5}\s*right\s*$/.test(trimmed)) {
      flushPara()
      closeList()
      if (!pendingLeftWrapper) html.push('<div class="lr-container">')
      pendingLeftWrapper = false
      html.push('<div class="right">')
      stack.push('right')
      continue
    }
    if (/^:{3,5}\s*header\s*$/.test(trimmed)) {
      beforeNormal()
      html.push('<section class="resume-header">')
      stack.push('header')
      continue
    }
    if (/^:{3,5}\s*title\s*$/.test(trimmed)) {
      beforeNormal()
      html.push('<div class="resume-title-block">')
      stack.push('title')
      continue
    }
    ...
    if (ordered || list) {
      ...
      if (listTag !== tag) {
        closeList()
        closePendingLeft()
        html.push(`<div class="r-list r-list-${tag}">`)
        listTag = tag
        listIndex = 0
      }
      listIndex += 1
      const marker = tag === 'ol' ? `${listIndex}.` : '•'
      html.push(
        `<div class="r-li"><span class="r-li-marker">${marker}</span><span class="r-li-body">${inline((ordered || list)[1])}</span></div>`,
      )
      continue
    }
    closeList()
    if (pendingLeftWrapper) closePendingLeft()
    para.push(trimmed)
  }
  flushPara()
  closeList()
...
export function applyResumePhotoToHtml(renderedHtml, photoUrl, transform = {}, options = {}) {
  const url = sanitizeInlineUrl(photoUrl, true)
...
function findPhotoTarget(root) {
  const profileHeading = Array.from(root.querySelectorAll('h2,h3,h4')).find((heading) =>
    /个人(资料|信息|简历)|基本信息/.test(heading.textContent || ''),
  )
...
function photoMarkup(url, transform = {}, options = {}) {
  const safeUrl = sanitizeInlineUrl(url, true)
  if (!safeUrl) return ''
  return `<div class="resume-floating-photo"><span class="resume-photo-frame${options.selected ? ' is-selected' : ''}" data-managed-resume-photo="true" style="${photoTransformStyle(transform)}"><img class="resume-photo" alt="${MANAGED_RESUME_PHOTO_ALT}" src="${escapeHtml(safeUrl)}" draggable="false">${photoHandlesMarkup(options.selected)}</span></div>`
}
...
export function iconSvg(name) {
  ...
  const paths = {
    ...
    github:
      '<path d="M9 19c-5 1-5-2-7-3m14 6v-4c0-1 0-2-1-3 3 0 6-1 6-6 0-1 0-3-1-4 0-1 0-2 0-4 0 0-1 0-4 2-2-1-6-1-8 0-3-2-4-2-4-2 0 2 0 3 0 4-1 1-1 3-1 4 0 5 3 6 6 6-1 1-1 2-1 3v4"/>',
    ...
    twitter:
      '<path d="M22 5c-1 1-2 1-3 1 1-1 1-2 1-3-1 1-2 1-3 1a4 4 0 0 0-7 3v1A11 11 0 0 1 3 4s-4 9 5 13c-2 1-4 1-6 1 9 5 20 0 20-11V5z"/>',
    ...
    database:
      '<ellipse cx="12" cy="5" rx="8" ry="3"/><path d="M4 5v14c0 2 4 3 8 3s8-1 8-3V5M4 12c0 2 4 3 8 3s8-1 8-3"/>',
    server:
      '<rect x="4" y="4" width="16" height="6" rx="1"/><rect x="4" y="14" width="16" height="6" rx="1"/><path d="M8 7h.01M8 17h.01"/>',
    ...
    chart:
      '<path d="M4 19V5M4 19h17"/><rect x="7" y="11" width="3" height="5"/><rect x="12" y="8" width="3" height="8"/><rect x="17" y="4" width="3" height="12"/>',
