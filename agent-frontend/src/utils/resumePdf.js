...
export function photoResizeDelta(handle, dx, dy) {
  let delta = 0
  let count = 0
  if (handle.includes('e')) { delta += dx; count += 1 }
  if (handle.includes('w')) { delta -= dx; count += 1 }
  if (handle.includes('s')) { delta += dy; count += 1 }
  if (handle.includes('n')) { delta -= dy; count += 1 }
...
export function getPdfLinkLayouts(pageEls) {
  return pageEls.map(pageEl => {
    ...
    return Array.from(pageEl.querySelectorAll('a[href]')).map(link => {
...
export function addPdfLinks(pdf, layouts) {
  layouts.forEach(item => {
    pdf.link(item.leftRatio * 210, item.topRatio * 297, item.widthRatio * 210, item.heightRatio * 297, { url: item.href })
  })
}
