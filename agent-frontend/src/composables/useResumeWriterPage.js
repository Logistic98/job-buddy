...
export function useResumeWriterPage() {
  ...
  async function buildHtmlDocument() {
    ...
    const body = sanitizeResumeHtml(
      applyResumePhotoToHtml(
        renderResumeMarkdown(markdown.value),
        dataUrl || displayPhotoUrl.value,
        photoSettings.value,
      ),
    )
...
.export-group button{
  ...
  return {
    ...
    normalizeLineHeightInput,
    normalizeInitialPhotoUrl,
    isLocalOnlyPhotoUrl,
    readPhotoLibrary,
    readPhotoSettings,
    normalizePhotoSettings,
    ...
    saveDraft,
    importMarkdown,
    initializeVersionBaseline,
    markdownChangedSignificantly,
