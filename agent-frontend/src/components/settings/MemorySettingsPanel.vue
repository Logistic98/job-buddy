    ...
    <section v-if="memory" class="settings-section">
      <div class="setting-card">
        ...
        <div class="form-grid">
          ...
          <label
            ><span>最大记忆条数</span
            ><input v-model.number="memory.maxItems" type="number" min="20" max="1000" step="10"
          /></label>
        </div>
      </div>
      <div class="setting-card">
        ...
        <div class="memory-editor">
          ...
          <input
            ...
            placeholder="例如：优先看上海，Java 大模型应用开发，薪资 40-50k，排除外包驻场"
            @keyup.enter="create"
          />
          <button class="primary-btn" :disabled="!form.content" @click="create">新增记忆</button>
        </div>
...
const { value: memory, loading, saving, error, dirty, load, save } = useScopedSettings('memory', normalizeMemory)
const memories = ref([])
const form = ref({ type: 'preference', content: '' })
