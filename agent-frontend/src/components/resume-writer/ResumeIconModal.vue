<template>
  <div class="resume-modal-mask" @click.self="$emit('close')">
    <section class="resume-modal-card icon-modal-card">
      <header><div><h3>图标快捷写法</h3><p>点击图标会自动复制 <code>icon:name</code>，并插入到当前 Markdown 光标位置。</p></div><button @click="$emit('close')">×</button></header>
      ...
      <div class="icon-pager">
        ...
        <button v-for="page in iconPageCount" :key="page" :class="{ active: iconPage === page }" @click="iconPage = page">{{ page }}</button>
...
function safeIconSvg(icon) { return sanitizeResumeHtml(iconSvg(icon)) }

const ICON_PAGE_SIZE = 20
const iconOptions = [
  'info', 'phone', 'email', 'blog', 'github',
  'weixin', 'yuque', 'zhihu', 'gitee', 'weibo',
  'qq', 'twitter', 'facebook', 'csdn', 'juejin',
  'tag', 'sun', 'star', 'code', 'project',
  'user', 'briefcase', 'company', 'school', 'certificate',
  'award', 'skill', 'tool', 'database', 'server',
  'cloud', 'robot', 'chart', 'rocket', 'target',
  'location', 'calendar', 'time', 'money', 'link',
  'download', 'upload', 'edit', 'check', 'shield',
  'heart', 'flag', 'light', 'book', 'search',
  'message', 'meeting', 'growth', 'terminal', 'bug',
  'language', 'java', 'python', 'api', 'portfolio',
]
...
const pagedIcons = computed(() => filteredIcons.value.slice((iconPage.value - 1) * ICON_PAGE_SIZE, iconPage.value * ICON_PAGE_SIZE))
watch(iconKeyword, () => { iconPage.value = 1 })
watch(iconPageCount, count => { if (iconPage.value > count) iconPage.value = count })
...
.icon-grid button{border:1px solid #dfe6f2;background:#fff;color:#475467;border-radius:12px;display:flex;align-items:center;gap:8px;justify-content:flex-start;padding:10px 12px;font-size:12px;font-weight:800}
</style>
