<template>
  <aside :class="['app-sidebar', { collapsed }]">
    <div class="brand-row">
      ...
      <div class="workspace-name" aria-label="JobBuddy"><span>Job</span><strong>Buddy</strong></div>
      <button
...
function navigate(item) {
  if (item.externalUrl) {
    if (/^https?:\/\//i.test(item.externalUrl)) window.open(item.externalUrl, '_blank', 'noopener,noreferrer')
    return
  }
  if (item.to && route.path !== item.to) router.push(item.to)
}
...
const healthLabel = computed(
  () => healthState.value.label || (healthState.value.status === 'up' ? '系统就绪' : '服务未连接'),
)
...
const icons = {
  workbench: [
    'M4 5.5A2.5 2.5 0 0 1 6.5 3h11A2.5 2.5 0 0 1 20 5.5v13A2.5 2.5 0 0 1 17.5 21h-11A2.5 2.5 0 0 1 4 18.5z',
    'M8 7h8',
    'M8 11h8',
    'M8 15h5',
  ],
  ...
  projectDeep: [
    'M4 6.5A2.5 2.5 0 0 1 6.5 4H10l2 2h5.5A2.5 2.5 0 0 1 20 8.5v9A2.5 2.5 0 0 1 17.5 20h-11A2.5 2.5 0 0 1 4 17.5z',
    'M8 11h8',
    'M8 15h5',
    'M15 15l2 2 3-5',
  ],
...
const navItems = computed(() =>
  auth.menus
    ...
    .map((menu) => ({
      key: menu.menuId || menu.menuCode,
      to: menu.routePath || '',
      externalUrl: menu.externalUrl || '',
      label: menu.menuName || menu.menuCode,
      iconPaths: icons[menu.iconKey] || icons.folder,
    })),
)
</script>
