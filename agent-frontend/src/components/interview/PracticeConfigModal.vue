<template>
  <div
    ...
    :class="embedded ? 'rule-practice-panel glass-card' : 'modal-mask'"
    @click.self="handleBackdrop"
  >
    <div :class="embedded ? 'practice-config-embedded' : 'modal-card interview-modal-card practice-create-modal'">
      <button v-if="!embedded" class="close" @click="close">×</button>
      <div class="practice-modal-head">
        <h2>规则组卷</h2>
        <p>设置练习名称、限时和练习模式，再按题库、分类、难度、题型确定规则并组合抽题。</p>
      </div>
      ...
      <div class="practice-form">
        ...
        <div class="practice-section practice-duration-section">
...
const durationPresets = [15, 30, 45, 60, 90, 120, 150, 180, 240]
...
async function startExam() {
  ...
  try {
    ...
    practiceModalError.value = err.message || '规则组卷失败'
  } finally {
