...
Runtime 任务理解在能力分类前先执行上下文重写。Backend 提供最近用户/助手消息以及结构化 `previous_slots`，Runtime 将“现在这份简历呢”等省略式输入解析为独立可执行任务，并输出解析后的引用来源；意图分类必须基于重写后的完整语义，而不是只匹配当前短句。历史信息只能补全当前请求，不能覆盖用户本轮明确给出的新目标。能够唯一解析时继续执行，存在多个候选或缺少可靠来源时进入澄清。
