...
浏览器链路使用 `session`、`intent_precheck`、`intent`、`tool_status`、`reasoning_delta`、`message_delta`、`message`、`job_cards`、`resume_match`、`auth_required`、`error`、`heartbeat` 和 `done` 等事件。Runtime 返回的 directive 是 Backend 内部两阶段编排结果，不作为独立浏览器事件；其摘要通过 `tool_status` 和 `intent` 暴露。`message_delta` 与 `reasoning_delta` 分别承载答案和过程增量，`message` 用于终态校正，岗位和简历结构化结果使用专用事件。`done` 只携带 `ok`，表示 Backend 已等待本轮消息与状态落库后完成收尾；Trace、终止原因、指标和持久化元数据保存在消息、工具事件和会话状态中，不重复塞入 `done`。
