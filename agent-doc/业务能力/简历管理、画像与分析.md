...
`POST /api/resume/upload` 只完成文件校验、对象存储写入和 `resume_record` 落库，成功后立即返回 `parseStatus=pending` 的简历摘要。上传请求不得同步调用 Runtime 做解析或分析，也不得让工作区状态保存、列表对账等后续操作继续占用前端“上传中”状态。内容解析与竞争力分析由用户发起的持久化后台分析任务完成；聊天、岗位匹配等需要结构化简历的链路可在自身执行边界内按需解析。
