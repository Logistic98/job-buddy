...
勾选数量不设业务上限。确认导入后 Backend 按选择顺序串行复用完整快照保存逻辑，仅此时按需读取岗位详情 JD；任何登录、风控、限速、依赖或详情完整性异常都会停止剩余项，避免连续失败扩大账号风险。Tool 的详情小时/日配额和冷却仍是最终保护边界，因此大量选择可能形成部分成功，其余项会明确标记为未处理。已完成项不回滚，并返回逐项状态与最新本地收藏列表。该流程只写本地 `job_favorite`，不会修改 Boss 收藏状态。

接口契约为：`GET /api/jobs/favorites/boss?page=1` 返回去重后的单页摘要、`page`、`totalPages`、`hasMore` 和只读配额摘要；`POST /api/jobs/favorites/boss/import` 请求体为 `{ "jobs": [...] }`，响应包含 `importedCount`、`existingCount`、`failedCount`、`unprocessedCount`、`stopped`、`authRequired`、`items` 与 `favorites`。外部岗位对象在 Backend 以 `JsonNode` 明确隔离，不在 Controller、Service 或 Client 新增无 schema 的 Map DTO。
...
收藏前必须形成完整岗位快照。列表结果已有 JD 时直接标准化为 `jobDescription`；缺失时在收藏请求内根据 `securityId` 或原岗位链接调用详情能力。详情定位信息缺失、登录失效、采集失败或结果仍无 JD 时，不写入不完整收藏。查询收藏和分析只读取固化快照，不临时访问 Boss。

收藏记录和临时分析请求都必须包含完整 JD；已收藏岗位分析只读取固化的 `job_json`，不得用分析请求改写岗位快照。分析结果写入独立的 `analysis_json` 和 `analyzed_at`，不得混入岗位快照。
...
测试覆盖已有 JD 不重复采集、缺失 JD 时补全、失败不入库、Boss 不限量选择、本地收藏整页去重、Boss 页内去重、部分成功、首个外部异常后停止、历史快照修复、跨用户拒绝、空结果不写入、活动任务复用、部分结果持久化、断线恢复和最终覆盖。前端需验证收藏回滚、导入弹窗逐项勾选、分页替换、全部勾选、已导入隐藏、未登录同窗二维码、扫码轮询不重叠、登录后单次首屏加载、加载与错误状态、报告响应式布局、重试和刷新恢复。
