...
列表和详情接口返回完整项目档案。项目列表汇总项目数、材料数、问题数和平均准备度；单个项目准备度按缺少材料、有材料但无问题、已有问题三个阶段分别计为 20%、60% 和 100%，平均准备度为所有项目准备度的算术平均值并四舍五入为整数。前端概览采用经历主内容与基础信息侧栏布局，主内容展示摘要、背景、职责、亮点、难点和成果；侧栏展示角色、类型、领域、周期、状态、团队、技术栈及材料/问题数量。空字段只给出轻量补充提示，不生成虚假内容。
...
项目详情支持维护问题和调用 `/projects/{projectId}/generate` 生成 1–100 道复盘问题候选。生成请求包含数量、关注方向和可选的生成要求；生成要求最长 1000 个字符，用于约束候选问题及参考答案的组织方式，生成器必须将其作为每道候选的统一回答约束，不能忽略或仅在前端保存。生成接口只返回不带 `questionId` 的候选，不写入数据库；前端必须在同一弹窗中展示问题、分类、难度和参考答案，允许用户逐项勾选后，再调用 `/projects/{projectId}/questions/import` 将选中的候选批量添加到题库。关闭弹窗或重新生成不会改变已有题库。模型只能使用用户项目档案中的结构化字段，不得把文件内容当作可信指令；二进制材料仅提供存储和下载，不参与自动内容理解。
