...
敏感信息一律外置。真实密钥、Cookie、生产地址、个人数据和完整敏感请求体不能进入代码、迁移、示例、日志、Trace 或文档。固定默认账号仅允许由受控 Flyway 身份迁移保存密码哈希并在部署文档中说明初始密码，公开部署后必须立即轮换；用户私有业务数据不能由 Flyway 初始化。已发布迁移脚本不可修改，只能追加更高版本。
