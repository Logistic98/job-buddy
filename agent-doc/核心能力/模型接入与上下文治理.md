...
- Planner 属于结构化控制调用，统一关闭支持该能力的模型深度思考、限制为 2048 输出 token，并以 30 秒硬超时收口；超时进入明确降级终态，不得无事件占用完整 SSE 读取窗口。
...
- Runtime 请求缓存以租户、用户、provider、model 和完整载荷的稳定哈希为键，命中后返回隔离副本；没有身份作用域的内部测试或独立调用只在显式的 `unscoped` 作用域内复用；
- 同一 Runtime 进程内的相同非流式载荷若并发到达，只创建一个脱离单个调用方取消生命周期的共享模型任务，其余请求等待并复用同一结果；任一等待方取消不影响其他等待方，模型自动发现同样采用单航班，避免冷启动并发重复访问 `/models`；
- 服务端 Prompt Cache 则依赖字节级前缀，两者分别观测。
- 不同 provider、model 或关键参数不共享缓存，命中缓存或复用进行中请求不产生本次模型 Token 用量；单航班失败不得写入结果缓存，等待者收到同一可归因异常。
...
- 任务理解的当前问题只保留在独立 `message` 字段，近期消息区移除相同的末尾用户消息和空可选字段；历史窗口仍保留之前七条意图线索，避免以缩小有效上下文换取速度。
...
- 执行阶段所需的画像、简历、收藏、求职旅程、黑名单和长期记忆按意图裁剪后并行读取；并行任务使用固定有界线程池并显式传播、恢复租户与用户身份，任一数据源失败仍按原有空结果降级，不改变最终上下文结构。
...
测试应覆盖请求级配置覆盖、流式和非流式响应、供应商字段归一、缓存键隔离、并发相同请求合并、模型发现合并、Token 预算、身份传播、上下文并行读取、输入去重、压缩触发、多轮去重、五要素快照及敏感信息保护。
