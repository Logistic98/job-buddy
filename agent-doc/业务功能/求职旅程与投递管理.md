...
投递记录包含公司与岗位快照、企业简介、收藏关联键、业务方向、面试轮次和时间、内容与形式、结果、复盘、JD、流程、下一步、状态、优先级和标签。记录可以独立创建，也可通过 `favoriteKey` 关联收藏；关联不改变所有权，删除收藏不会级联删除旅程。

求职进展弹窗按“岗位概述、任职要求、面试内容、复盘跟进”分组展示。“岗位概述”包含企业与岗位信息，并以企业简介承接业务、团队和产品背景；任职要求独立呈现；面试轮次、时间、形式、结果、状态、优先级与面试正文集中在“面试内容”分组；反思总结与下一步动作按上下结构呈现。界面将历史记录中的面试内容与面试流程合并为一个“面试内容”字段，编辑保存时写入 `interviewContent`，并清空已合并的 `interviewProcess`，以兼容存量数据且避免重复展示。
