...
- `POST /api/resume/upload` 完成校验、对象写入和记录落库，并基于请求体尽力预生成 PDF 首页缩略图缓存，随后返回 `parseStatus=pending`；缩略图预热失败不回滚原文件，首次读取时仍会按需补建。
- 上传请求不调用 Runtime，也不让工作区保存或列表对账占用“上传中”；
- 内容解析与竞争力分析由持久化后台任务完成，聊天和岗位匹配可在自身边界内按需解析。
- 删除简历前必须先取消同一用户、同一简历的活动分析任务，再删除 MinIO 对象和数据库记录；前端同时终止该简历的 SSE / 轮询订阅，禁止已删除资源的迟到事件污染下一份简历。
- MinIO 下载到 Runtime 工作区时必须清理失败产生的临时文件，并对流式网络中断做一次有界重试；对象不存在、权限或签名错误仍应立即失败，不得用重试掩盖确定性配置问题。
- 分析任务错误必须按 `resumeId` 归属并只在简历分析视图展示；简历列表的全局错误只承载上传、删除、列表刷新和元数据保存等管理操作失败，切换或删除当前简历不得暴露其他简历的历史分析错误。
- 当简历已有可见的成功分析报告时，后续历史失败任务不得与当前成功报告同时显示错误提示；只有当前没有可用报告时，才展示最近一次失败原因。

通过会改写 AWS V4 请求的对象存储代理时，`JOB_BUDDY_MINIO_SIGNATURE_VERSION=auto` 使用项目内 S3 V2 兼容签名；直连标准 MinIO/S3 时应显式使用默认的 `v4`。S3 V2 的 HTTP Date 固定使用 GMT 与两位日期，避免每月前九天被对象存储判定为 `MalformedDate`。
...
PDF 导出在浏览器内按页渲染并下载，开发环境必须在 Vite 启动时预构建 `html2canvas` 与 `jspdf`，禁止把首次导出变成依赖优化触发点，避免旧模块哈希导致动态导入失败。
...
画像表单存在未保存修改时，浏览器刷新或关闭页面沿用 `beforeunload` 提示；通过 Vue Router 站内离开或从侧边栏退出登录时必须显示明确确认，取消后留在当前页面，确认后才允许离开。
...
- 验证应覆盖 PDF-only、租户和用户隔离、资源所有权、版本裁剪与恢复、画像摘要编辑与差异确认、异步任务复用、删除时取消活动任务与前端订阅、MinIO 流式中断重试和临时文件清理、部分结果恢复、评分加权与边界归一化、评分证据门槛、不同维度输入的分数区分度和模型禁止编造。
- 前端交付必须用浏览器验证三视图、预览、上传、版本回退、画像概览、分析加载及刷新恢复。列表缩略图在生成或网络读取期间必须显示明确占位，首屏卡片优先加载，失败时允许用户重试，不得长时间展示无解释的空白区域。
