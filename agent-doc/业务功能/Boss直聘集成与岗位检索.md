...
- 用户给出的薪资闭区间完全落在单个 Boss 薪资桶时，Backend 将该桶下推到 `search.salary` 以减少无关召回；跨桶区间不得强行下推，精确薪资仍由本地规则复核。例如 `40-50K` 下推为 `30-50K`，然后按原始 `40-50K` 做精确过滤。
- 字段必须在 Runtime、Backend、工具契约和无网络测试中保持一致。

- Backend 将本轮明确条件与求职画像、当前简历合并，本轮条件优先。
- 岗位硬排除词只能检查岗位名称、职位描述、技能、标签和福利等用户可见业务字段；`securityId`、加密岗位 ID、图片地址、原始 URL 和其他不透明元数据不参与命中。`OD` 等短 ASCII 排除词按独立词或中文相邻语义匹配，不得命中 `model`、`product` 或随机令牌片段。
...
- 只有用户未限制薪资时才允许未知值。
- Boss 返回非空岗位列表但整批薪资展示值和结构化上下界均缺失时，Tool 必须把它归类为上游响应不完整并停止本轮，禁止让 Backend 全部过滤后伪装成“0 个候选岗位”。该异常不自动重试，避免增加真实 Boss 访问频率；用户可在上游状态恢复后手动重试。

岗位族、专项能力、经验和排除项同样执行硬过滤。岗位明确要求多模态、视觉、语音或推荐算法等专项能力，而画像与简历没有对应证据时不得进入推荐。

### 匹配质量门

- Backend 在进入 Boss 搜索前并行读取画像、当前简历和黑名单；槽位补全完成后，完整匹配简历快照在专用有界线程池中预取，并与保持串行的 Boss 候选搜索重叠执行。质量门必须复用该 Future，不得在搜索结束后重复同步读取和准备同一份简历。确定性 `job.recommend` 不检索不会参与筛选槽位计算的长期记忆，避免旁路超时阻塞首屏。
