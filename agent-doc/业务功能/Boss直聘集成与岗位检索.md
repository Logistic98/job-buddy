...
- 临时令牌失效时先静默补齐；
- 一次性 Chromium 页面提前关闭可以用全新临时上下文重试一次。
...
- 默认首批展示 5 个岗位、候选池倍率为 3、总评分上限为 30；常规首屏只需准备约 15 个候选，模型预筛按最多 4 个岗位的小批次依次补足展示槽位，避免大批结构化输出不完整后再拆分重试，更多结果由质量门续评或“换一批”按需消费。
...
- 通过硬过滤的候选使用当前简历按最多 4 个岗位的小批次执行 `resume_match`，证据模式固定为 `recommendation_list`。
...
- “查看职位描述”只展开或收起卡片已有 JD，是纯前端展示操作；“分析此岗位”只把岗位标识、名称、公司、条件标签、原链接和最多 2400 字 JD 等必要字段发送到聊天流。两个按钮不共享加载态，也不调用岗位详情接口。
- Backend 使用强类型 DTO 再次校验并忽略上游原始对象；分析入口只合并当前卡片与会话候选快照，不二次请求 Boss 详情。若检索阶段因外部失败仍未取得 JD，卡片明确提示重新检索，并拒绝创建无证据的精确分析轮次。
- 卡片正文展示岗位基础信息和业务标签；
...
- 搜索与详情默认小时配额均为 60，日配额分别保持 120 与 60；小时配额需要覆盖一次推荐任务的续页搜索和最终岗位 JD 补全，日配额、串行抖动、上游限流分类、连续失败硬停与风控冷却仍共同约束真实访问。
...
前端变更需要验证扫码续跑只保留一个用户轮次、换一批、最终卡片已携带 JD、职位描述纯前端展开/收起、分析按钮不二次请求详情、当前页选择、同窗二维码、登录后单次首屏加载、部分成功、错误收尾和凭据不落盘。
